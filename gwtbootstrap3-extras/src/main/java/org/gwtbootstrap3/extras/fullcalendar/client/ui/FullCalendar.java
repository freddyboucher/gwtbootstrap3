...
public class FullCalendar extends FlowPanel implements HasLoadHandlers {
  ...
  private void renderCalendar() {
    ...
    addCalendar(getElement().getId(), currentView.name(), editable, selectable, selectHelper, unselectAuto, selectOverlap, language,
        timezone, weekNumberTitle, unselectCancel, selectContraint, javascriptParams);
  ...
  private void ensureInjected(Language language) {
    ...
    JavaScriptObject scriptElement =
        ScriptInjector.fromString(language.getResource().getText()).setWindow(ScriptInjector.TOP_WINDOW).inject();
    languageScripts.put(language.getCode(), scriptElement);
  }

  private native void addCalendar(String id, String currentView, boolean editable, boolean selectable, boolean selectHelper,
                                  boolean unselectAuto, boolean selectOverlap, String lang, String timezone, String weekNumberTitle,
                                  String unselectCancel, String selectContraint, JsArray<JavaScriptObject> options) /*-{

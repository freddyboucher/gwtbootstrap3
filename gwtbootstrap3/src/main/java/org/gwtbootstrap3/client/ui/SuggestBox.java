...
public class SuggestBox extends com.google.gwt.user.client.ui.SuggestBox
    implements HasId, HasResponsiveness, HasPlaceholder, HasAutoComplete, HasSize<InputSize>, HasEditorErrors<String>, HasErrorHandler,
  ...
  static class CustomSuggestionDisplay extends DefaultSuggestionDisplay {
     ...
     */
    @Override
    protected void showSuggestions(com.google.gwt.user.client.ui.SuggestBox suggestBox, Collection<? extends Suggestion> suggestions,
      ...
      if (!suggestBox.getElement().getStyle().getZIndex().equals("")) {
        try {
          getPopupPanel().getElement().getStyle().setZIndex(Integer.parseInt(suggestBox.getElement().getStyle().getZIndex()));
        } catch (Exception e) {
          // Do nothing. We tried....
        }
      }
  ...
  private final BlankValidatorMixin<SuggestBox, String> validatorMixin =
      new BlankValidatorMixin<>(this, errorHandlerMixin.getErrorHandler());
